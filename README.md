...
- **매수 후보 추천**: 포트폴리오 공백 섹터를 진단해 스크리닝 기반 매수 후보 제안 (판단만, 주문 안 함)
- **자동매매 준비 중**: 매수 규칙 A/B(점수집중/분산채우기)·손절·익절 규칙 판단, 가드레일 검사(kill switch·시간·비중·횟수 제한) — **판단/제안만 수행하며 실제 자동 주문은 실행하지 않습니다**
- **리스크 검수**: 직원들의 분석을 교차 검증해 숫자 오류·과장·환각을 걸러냄
...
| 추천 (Advisor) | 포트폴리오 공백 섹터 진단 후 매수 후보 제안 (단정 금지, 판단만) |
