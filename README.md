...
- **KIS 모의 계좌 연동**: 모의투자 계좌의 국내·미국 보유종목과 잔고를 실시간 조회. 주문 전송은 `auto_trader.py`를 통해서만 수행
...
- **자동매매 실험(모의 전용)**: 매수 규칙 A/B(점수집중/분산채우기)·손절/익절 규칙 판단 → 가드레일 검사(kill switch·거래일/거래시간·비중·횟수 제한) → KIS 모의계좌 주문 전송을 `auto_trader.py`가 조립 실행. 단계별 안전장치(상수 세트, .env 확인 핀, 킬 스위치, 1회 주문 상한) 하에서만 동작
- **리스크 검수**: 직원들의 분석을 교차 검증해 숫자 오류·과장·환각뿐 아니라 직원 간 모순·자기모순·빠진 관점까지 점검
- **대화 맥락 기억**: 이전 질문을 기억해 "그럼 그건?" 같은 이어지는 질문 처리

### 자동매매 진행 단계

자동매매 실험은 관찰(0단계) → 승인 모드(3단계) → 승인 면제(4단계) 순으로 단계를 밟아 진행하며,
현재 단계는 `auto_trader.py`의 상수 세트(`PHASE`/`MASTER_ENABLE`/`DRY_RUN`/`APPROVAL_REQUIRED`)로 관리됩니다.
(2026-07 기준: 4단계 — 승인 면제 모드, KIS 모의계좌 실주문, 킬 스위치·가드레일 상시 적용)
...
| 매매 규칙 (Trading Rule) | 규칙 A/B 매수 판단·손절·익절 분류 (판단만 — 주문은 auto_trader.py 전용) |
...
> 웹 UI(직원들)는 어떤 경우에도 주문을 내지 않습니다. 실제 주문 전송은
> 별도 프로그램인 `auto_trader.py`를 터미널에서 직접 실행할 때만 일어납니다.
...
> 이 외에도 `.dart_corp_codes.json`(DART 기업코드 캐시), `trade_log.json`·`trade_log_TEST.json`(거래 로그),
> `auto_trader_dryrun_log.json`(dry-run 로그), `kill_switch_state.json`(킬 스위치 상태)이 `.gitignore`에 등록되어 있습니다.
...
웹 UI는 분석·판단 전용이며 주문 기능이 없습니다.

### 5. 실행 — 자동매매 (모의 전용, 별도 안전장치 필요)

```bash
caffeinate -i ./venv/bin/python auto_trader.py --rule ALL --market KR
```

단, 아래 안전장치를 전부 통과해야만 실제로 동작합니다.

- `auto_trader.py`의 상수 4개가 허용된 세트 조합이어야 함 (어중간한 조합이면 즉시 중단)
- `.env`에 현재 단계에 맞는 확인 핀(`PHASE3_CONFIRM` 또는 `PHASE4_CONFIRM`)이 정확한 문구로 존재해야 함 — 없으면 기동 자체가 차단됨
- 허용된 모의계좌 번호가 하드코딩으로 검증되며, 매수 1회 200만 원 상한 적용 (기본 예산은 1건당 50만 원, 현재가 200만 원 이내 고가주는 1주 예외 허용 — 2026-07-31 옵션2)
- 킬 스위치(일일 -5% / 누적 -15%) 발동 시 전체 주문 차단

> 저장소를 클론만 한 상태에서는 확인 핀이 없어 자동매매가 실행되지 않는 것이 정상입니다.
...
# 매매 규칙 판단 (웹 UI에서는 판단만 — 주문은 auto_trader.py 전용)
규칙 A는 뭘 사라고 해?
규칙 B로 분산 채우기 후보 알려줘
지금 손절 대상 있어?
```
...
```
stock-agent/
├── tools.py              # 데이터·지표·뉴스·재무·거시·스크리닝 + 자동매매 부품 (KIS 주문/가드레일/로그)
├── employees.py          # 직원(에이전트) 정의
├── manager.py            # 총괄 매니저 + 실행 로직
├── app.py                # Streamlit 웹 UI
├── auto_trader.py        # 자동매매 조립 프로그램 (부품 연결 전용 — 단계별 안전장치 내장)
├── test_manager.py       # 매니저 동작 테스트
├── scripts/
│   └── export_transcripts.py  # 대화 기록 내보내기 (Claude Code JSONL → 마크다운)
├── requirements.txt      # 필수 라이브러리 목록
├── trade_log.json        # A/B 매매 실험 장부 (규칙별 딱지 기록, 자동 생성, git 제외)
├── auto_trader_dryrun_log.json  # dry-run 전용 별도 로그 (자동 생성, git 제외)
├── kill_switch_state.json       # 킬 스위치 상태 (자동 생성, git 제외)
...
- **역할 분리**: 각 직원은 자기 전용 도구만 사용하며 역할을 섞지 않음. 주문은 조립 프로그램(auto_trader.py) 전용
- **정직성 우선**: 미래 주가 예측을 하지 않으며, 모든 분석은 "현재 데이터 기반 참고용"
- **견고성**: 무료 데이터 소스의 간헐적 실패에 대비해 캐싱·재시도·부분 성공 처리. 판단에 필요한 데이터를 못 구하면 근사치로 진행하지 않고 중단(fail-safe)
- **안전성(코드로 강제)**: 자동매매는 KIS 모의계좌 전용(허용 계좌 하드코딩). 단계 전환은 상수 세트 단위로만 가능하고, .env 확인 핀이 없으면 기동 차단. 킬 스위치(일일 -5% / 누적 -15%, 누적은 수동 해제만)·자금 한도·비중·횟수 제한이 주문 전 항상 검사됨
- **데이터 먼저, 서두르지 않기**: 각 단계는 충분한 관찰 데이터와 명시적 승인을 거친 뒤에만 진행
