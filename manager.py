...
def manager(user_input: str, history: list = None, status_callback=None) -> str:
    ...
    system_prompt = (
        ...
        "   ⚠️ **추천 직원(call_advisor_employee)을 호출한 경우에는 리스크 검수가 필수입니다.** "
        "추천 직원 결과 전체를 call_risk_review_employee의 reports 인자로 넘겨 검증받아야 합니다. "
        "검수 지시 시 다음 항목을 명시하세요: "
        "(1) 추천 근거 수치(영업이익률·성장률 등)에 과장·오류 없는지, "
        "(2) '반드시 사라', '강력 추천' 등 단정적 매수 권유 표현이 포함됐는지, "
        "(3) 흑자전환 종목에 기저효과 주의 경고가 빠지지 않았는지, "
        "(4) 면책 고지(투자 권유 아님 문구)가 포함됐는지. "
        "이 검수는 추가 외부 데이터 조회 없이 텍스트만 보고 빠르게 수행하면 됩니다.\n"
        ...
        if response.stop_reason == "tool_use":
            ...
            tool_results = [
                {"type": "tool_result", "tool_use_id": b.id,
                 "content": f"[중복 제거] {b.name}은(는) 이 라운드에서 이미 처리되었습니다."}
                for b in skipped_blocks
            ]

            tool_blocks = active_blocks
            # ────────────────────────────────────────────────────────────
