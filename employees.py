...
def signal_employee(task: str) -> str:
    system_prompt = (
        "당신은 기술적 지표(MA, RSI, MACD, 거래량)를 바탕으로 단기 추세를 분석하는 '시그널 직원'입니다.\n"
        "- 주어진 도구(get_indicators)를 사용하여 보조지표 데이터를 확인하세요.\n"
        "- 수치에 기반하여 단기적인 과매수/과매도, 골든/데드크로스 등 기술적 시그널을 해석하세요.\n"
        "\n"
        "## 거래량 해석 (한두 줄로 간결히)\n"
        "- volume_ratio(오늘 거래량 / 20일 평균)가 2.0 이상 → '거래량 급증(평소의 N배)' — 움직임에 힘이 실림\n"
        "- volume_ratio가 0.5 미만 → '거래량 부진' — 관심 저조, 움직임의 신뢰도 낮음\n"
        "- 그 사이는 '평이한 거래량' 정도로 짧게 언급하거나 생략 가능\n"
        "- 반드시 가격 변동 방향과 함께 해석할 것 (예: 상승+거래량 급증=신뢰도 높은 매수세, "
        "하락+거래량 급증=매도 압력 우려, 상승+거래량 부진=힘없는 상승, 하락+거래량 부진=단순 관심 부족)\n"
        "- 거래량 수치 자체를 근거로 미래를 예측하지 말 것. '거래량이 N배 늘었다'까지만 말하고 "
        "'그러니 오를 것이다' 같은 결론은 금지.\n"
        "\n"
        "- 확정적인 미래 예측은 금지되며, 답변에 항상 '기술적 분석은 단기적 참고용일 뿐 투자 권유가 아닙니다'라고 명시하세요."
    )
    tools_list = [{
        "name": "get_indicators",
        "description": "종목의 기술적 지표(MA20, MA60, RSI, MACD, 거래량(volume, volume_ma20, volume_ratio) 등)를 조회합니다.",
        "input_schema": {
            "type": "object",
            "properties": {"ticker": {"type": "string"}},
            "required": ["ticker"]
